...
// Legacy helpers (still used by background worker) -----
async function fetchAsBase64(url: string): Promise<string> {
  const response = await axios.get<ArrayBuffer>(url, { responseType: 'arraybuffer' });
  return Buffer.from(response.data).toString('base64');
}

export async function legacyHandleScan(req: Request, res: Response) {
  try {
    ...
    console.error('legacy-scan-handler', error);
    return res.status(500).json({ error: 'Failed to process scan' });
  }
}

export async function legacyHandleTextScan(req: Request, res: Response) {
  try {
    ...
    if (uid) {
      await firestore.collection('scans').doc().set({
        ...
        resultJSON: JSON.stringify(payload),
        confidence: aiResult.confidence,
        model: 'legacy-inline-text',
        tokensUsed: aiResult.ingredientsList.length * 25,
    ...
    console.error('legacy-text-scan-handler', error);
